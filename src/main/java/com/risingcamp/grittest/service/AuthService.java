...
import com.risingcamp.grittest.controller.admin.user.dto.AdminLoginRequestDto;
import com.risingcamp.grittest.controller.admin.user.dto.AdminSingUpRequestDto;
...
public class AuthService {
    ...
    @Transactional
    public void adminSignup(AdminSingUpRequestDto request) {
        if (userRepository.existsByLoginId(request.getLoginId())) {
            throw new BaseException(HttpStatus.CONFLICT, "이미 존재하는 아이디입니다.");
        }
    ...
    @Transactional
    public String adminLogin(AdminLoginRequestDto request) {
        String loginId = request.getLoginId();
        User user = userRepository.findByLoginId(request.getLoginId())
                .orElseThrow(() -> new BaseException(HttpStatus.NOT_FOUND, "데이터베이스 내 유저가 존재하지 않습니다. phone : " + loginId));
        // 소셜 로그인 확인 로직
